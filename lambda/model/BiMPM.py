...
class BiMPM(nn.Module):
    def __init__(self, pretrained_embedding, vocabs, char_vocabs,
                 ...
                 matching_dim=100, perspectives=5,
                 full_match=True, maxpool_match=True, att_match=True, maxatt_match=True,
                 aggregation_dim=100, aggregation_dropout=0.1,
                 prediction_dim=100, output_dim=2):

        super(BiMPM, self).__init__()

        self.word_layer = WordRepresentationLayer(pretrained_embedding, vocabs, char_vocabs, word_dim=word_emb_dim,
                                                  char_lstm_dim=char_emb_dim, char_dim=char_dim, dropout=0.1)
        ...
        self.matching_layer = MatchingLayer(hidden_dim=matching_dim, perspectives=perspectives,
                                            full_match=full_match, maxpool_match=maxpool_match,
                                            att_match=att_match, maxatt_match=maxatt_match)

        self.aggregation_layer = nn.LSTM(8 * perspectives, aggregation_dim,
    ...
    def forward(self, data):
        ...
        p, q = self.matching_layer(p, q), self.matching_layer(q, p)
